...
var routes = [
    { path: '', redirectTo: 'home', pathMatch: 'full' },
    {
        ...
        "children": [
            ...
            {
                path: "settings",
                loadChildren: function () { return Promise.resolve(/*! import() */).then(__webpack_require__.bind(null, /*! ./settings/settings.module */ "./src/app/settings/settings.module.ts")).then(function (m) { return m.SettingsModule; }); }
            }
        ]
    },
    {
...
var AppComponent = /** @class */ (function () {
    function AppComponent(commonSvc, authSvc) {
        this.commonSvc = commonSvc;
        this.authSvc = authSvc;
        this.title = 'app';
        if (!this.authSvc.isLogedIn) {
            this.commonSvc.initializeData().subscribe(function (res) {
                console.log(res);
            });
        }
    }
...
var depchartComponent = /** @class */ (function () {
    function depchartComponent(modalService, http, commonSvc, utilitySvc, baseUrl) {
    ...
    depchartComponent.prototype.ngOnInit = function () {
        title: 'Dep-Chart';
    ...
    depchartComponent.prototype.getTitle = function (id) {
        return this.commonSvc.getOptionTitle(id);
    };
    depchartComponent.prototype.fetchGroupChart = function () {
        var that = this;
        setTimeout(function () {
        ...
        function Deletegroups(nodeID) {
            ...
            if (nodeID < 999) {
                var afterDeletedData = data.filter(function (item) {
                    return !(item.id == nodeID || item.pid == nodeID);
                });
                localStorage.setItem('addtogrp', JSON.stringify(afterDeletedData));
                that.commonSvc.deleteGroup(nodeID).subscribe(function (isDeleted) {
                    if (isDeleted) {
                        that.utilitySvc.showNotification('success', 'Congratulation! Deleted Successfully');
                    }
                    else {
                        that.utilitySvc.showNotification('danger', "This " + _this.getTitle(1) + " name is already in use!");
                    }
                });
            }
            else {
    ...
    depchartComponent.prototype.addGroup = function (subgroupid) {
        var _this = this;
        if (subgroupid == undefined) {
            this.commonSvc.addGroup(this.group).subscribe(function (isAdded) {
                if (isAdded) {
                    _this.fetchGroupChart();
                    _this.modalService.dismissAll();
                    _this.utilitySvc.showNotification('success', 'Record added successfully!!');
                }
            });
        }
        else {
            this.commonSvc.addSubGroup(this.group, subgroupid).subscribe(function (isAdded) {
                if (isAdded) {
                    _this.fetchGroupChart();
                    _this.modalService.dismissAll();
                    _this.utilitySvc.showNotification('success', 'Record added successfully!!');
    ...
    depchartComponent.prototype.open = function (content, type, modalDimension, groupOption, group) {
        var _this = this;
        this.operationOption = groupOption;
        if (option.ADD == groupOption) {
            debugger;
            var headerTitle = "ADD " + this.getTitle(1);
            var buttonTitle = "Save";
            this.modelDialog = { headerTitle: headerTitle, buttonTitle: buttonTitle };
            this.group = new _services_Data_Groups__WEBPACK_IMPORTED_MODULE_4__["Group"]();
        }
        if (option.EDIT == groupOption) {
            var headerTitle = "EDIT " + this.getTitle(1);
            var buttonTitle = "Save changes";
            this.modelDialog = { headerTitle: headerTitle, buttonTitle: buttonTitle };
            this.group = group;
        }
        if (option.DETAIL == groupOption) {
            var headerTitle = this.getTitle(1) + " DETAIL";
            var buttonTitle = "";
            this.modelDialog = { headerTitle: headerTitle, buttonTitle: buttonTitle };
            this.group = group;
        }
        if (modalDimension === 'sm' && type === 'modal_mini') {
            ...
            this.modalService.open(content, { windowClass: 'modal-danger', centered: true }).result.then(function (result) {
                ...
                _this.closeResult = _this.getDismissReason(reason);
            });
        }
        else {
            debugger;
    ...
    depchartComponent.prototype.getDismissReason = function (reason) {
    ...
    depchartComponent.ctorParameters = function () { return [
...
var GroupsComponent = /** @class */ (function () {
    function GroupsComponent(modalService, commonSvc, utilitySvc) {
        this.modalService = modalService;
        this.commonSvc = commonSvc;
        this.utilitySvc = utilitySvc;
        this.searchText = "";
    ...
    GroupsComponent.prototype.getTitle = function (id) {
        return this.commonSvc.getOptionTitle(id);
    };
    GroupsComponent.prototype.getGroups = function () {
        var _this = this;
        this.commonSvc.getGroups().subscribe(function (res) {
            _this.model = res;
    ...
    GroupsComponent.prototype.deleteGroup = function (id) {
        var _this = this;
        this.commonSvc.deleteGroup(id).subscribe(function (isDeleted) {
            if (isDeleted) {
                _this.getGroups();
                _this.utilitySvc.showNotification('success', 'Congratulation! Deleted Successfully');
            }
            else {
                _this.utilitySvc.showNotification('danger', "This " + _this.getTitle(1) + " name is already in use!");
            }
        });
    };
    GroupsComponent.prototype.updateGroup = function () {
        var _this = this;
        this.commonSvc.updateGroup(this.group).subscribe(function (isUpdated) {
            if (isUpdated) {
                _this.getGroups();
                _this.modalService.dismissAll();
                _this.utilitySvc.showNotification('success', 'Record updated successfully!');
            }
        });
    };
    GroupsComponent.prototype.addGroup = function () {
        ...
        this.commonSvc.addGroup(this.group).subscribe(function (isAdded) {
            if (isAdded) {
                _this.getGroups();
                _this.modalService.dismissAll();
                _this.utilitySvc.showNotification('success', 'Love you jaan!!');
            }
        });
    };
    GroupsComponent.prototype.open = function (content, type, modalDimension, groupOption, group) {
        var _this = this;
        this.operationOption = groupOption;
        if (option.ADD == groupOption) {
            var headerTitle = "ADD " + this.getTitle(1);
    ...
    GroupsComponent.prototype.happyBirthday = function () {
        alert("HBD");
    };
...
var HomeComponent = /** @class */ (function () {
    function HomeComponent(commonSvc) {
        this.commonSvc = commonSvc;
        this.orgLevel = 0;
    }
    HomeComponent.prototype.ngOnInit = function () {
        ...
        this.commonSvc.getOrganizationLevel().subscribe(function (res) {
            _this.orgLevel = res;
        });
    };
...
var NavbarComponent = /** @class */ (function (_super) {
    ...
    function NavbarComponent(location, router, authSvc, element) {
        var _this = _super.call(this, authSvc) || this;
        _this.router = router;
...
}(_BaseLayoutComponent__WEBPACK_IMPORTED_MODULE_5__["BaseLayoutComponent"]));
...
var SidebarComponent = /** @class */ (function (_super) {
    ...
    function SidebarComponent(authSvc, commonSvc) {
        var _this = _super.call(this, authSvc) || this;
        _this.commonSvc = commonSvc;
        return _this;
    }
    SidebarComponent.prototype.ngOnInit = function () {
        this.menuItems = ROUTES.filter(function (menuItem) { return menuItem; });
    };
    SidebarComponent.prototype.getTitle = function (id) {
        var varia = this.commonSvc.getOptionTitle(id);
...
}(_BaseLayoutComponent__WEBPACK_IMPORTED_MODULE_1__["BaseLayoutComponent"]));
...
var LoginComponent = /** @class */ (function () {
    ...
    LoginComponent.prototype.login = function (form) {
        ...
        if (form.invalid)
            return true;
        this.authSvc.login(this.email, this.password).subscribe(function (isloggedIn) {
            if (isloggedIn) {
                _this.router.navigate(['/']);
            }
            else {
                _this.error = 'Invalid user and password!';
            }
        });
    };
...
var OrgChartComponent = /** @class */ (function () {
    function OrgChartComponent(modalService, http, utilitySvc, commonSvc, baseUrl) {
        //http.get<OrgCharts[]>(baseUrl + 'organization').subscribe(result => {
        //    this.orgData = result;
        //    let data = localStorage.getItem('orgChart');
        //    if (data == null || data && JSON.parse(data).length == 0) {
    ...
    OrgChartComponent.prototype.getTitle = function (id) {
        return this.commonSvc.getOptionTitle(id);
    };
    OrgChartComponent.prototype.fetchOrganization = function () {
        var that = this;
        setTimeout(function () {
        ...
        function callDetailHandler(nodeId) {
            ...
            that.commonSvc.getOrgChart(nodeId).subscribe(function (res) {
        ...
        function callRemoveHandler(nodeId) {
            that.nodeID = nodeId;
            that.throughGraph = false;
            that.commonSvc.removeFromOrganization(nodeId).subscribe(function (isRemoved) {
                if (isRemoved) {
    ...
    OrgChartComponent.prototype.addToOrganization = function () {
        var _this = this;
        if (!this.throughGraph && this.nodeID) {
            this.selectedOrgUser = this.nodeID;
        }
        this.commonSvc.addToOrganization(this.selectedUser, this.selectedOrgUser, this.selectedgroup).subscribe(function (isAdded) {
            if (isAdded) {
                _this.fetchOrganization();
                _this.modalService.dismissAll();
                _this.utilitySvc.showNotification('success', 'Record added to organization successfully!');
            }
        });
    };
    OrgChartComponent.prototype.addToGroup = function () {
        var _this = this;
        if (!this.throughGraph && this.nodeID) {
            this.selectedUser = this.nodeID;
        }
        this.commonSvc.addToGroup(this.selectedUser, this.selectedgroup).subscribe(function (isAdded) {
            if (isAdded) {
                _this.fetchOrganization();
                _this.modalService.dismissAll();
                _this.utilitySvc.showNotification('success', 'Added to Group successfully!');
            }
            else {
                _this.utilitySvc.showNotification('danger', 'Already added');
            }
        });
    };
    OrgChartComponent.prototype.onSelectOrg = function () {
        var _this = this;
        this.commonSvc.getOrgChart(this.selectedOrgUser).subscribe(function (res) {
            _this.activeOrgUser = res;
            console.log(res);
        });
    };
    OrgChartComponent.prototype.onselectUser = function () {
        var _this = this;
        this.commonSvc.getUser(this.selectedUser).subscribe(function (res) {
            _this.activeUser = res;
            console.log(_this.activeUser);
        });
    };
    OrgChartComponent.prototype.open = function (content, type, modalDimension, option) {
        var _this = this;
        if (option == 1) {
            this.throughGraph = true;
            this.addUToOrg = false;
        ...
        this.commonSvc.getUsersForOrganization().subscribe(function (res) {
            _this.users = res;
        });
        this.orgData = JSON.parse(localStorage.getItem('orgChart'));
        this.groups = JSON.parse(localStorage.getItem('Groups'));
    ...
    OrgChartComponent.prototype.getDismissReason = function (reason) {
        if (reason === _ng_bootstrap_ng_bootstrap__WEBPACK_IMPORTED_MODULE_2__["ModalDismissReasons"].ESC) {
            return 'by pressing ESC';
        }
        else if (reason === _ng_bootstrap_ng_bootstrap__WEBPACK_IMPORTED_MODULE_2__["ModalDismissReasons"].BACKDROP_CLICK) {
...
var OrgDemoComponent = /** @class */ (function () {
    function OrgDemoComponent(modalService, http, utilitySvc, _commonService, baseUrl) {
        this.modalService = modalService;
        this.http = http;
        this.utilitySvc = utilitySvc;
    ...
    OrgDemoComponent.prototype.ngOnInit = function () {
        this.users = JSON.parse(localStorage.getItem('users'));
        this.orgData = JSON.parse(localStorage.getItem('orgChart'));
        this.groups = JSON.parse(localStorage.getItem('Groups'));
    ...
    OrgDemoComponent.prototype.fetchDemoChart = function () {
        var that = this;
        ...
        diagram.events.on("ShapeIconClick", function (icon) {
            ...
            if (icon == "detail") {
                debugger;
                that._commonService.getDemoChart(id).subscribe(function (res) {
                    that.UserData = res;
    ...
    OrgDemoComponent.prototype.callAddHandler = function (id) {
        this.nodeID = id;
        this.addUToDemo = true;
        $('#addUToDemo').click();
    };
    OrgDemoComponent.prototype.callDetailHandler = function (id) {
        this.viewDetail = true;
        $('#viewDetail').click();
    };
    OrgDemoComponent.prototype.open = function (content, type, modalDimension, option) {
        var _this = this;
        if (option == 1) {
            this.addUToDemo = true;
        }
    ...
    OrgDemoComponent.prototype.getDismissReason = function (reason) {
        if (reason === _ng_bootstrap_ng_bootstrap__WEBPACK_IMPORTED_MODULE_2__["ModalDismissReasons"].ESC) {
            return 'by pressing ESC';
        }
        else if (reason === _ng_bootstrap_ng_bootstrap__WEBPACK_IMPORTED_MODULE_2__["ModalDismissReasons"].BACKDROP_CLICK) {
    ...
    OrgDemoComponent.ctorParameters = function () { return [
        { type: _ng_bootstrap_ng_bootstrap__WEBPACK_IMPORTED_MODULE_2__["NgbModal"] },
        { type: _angular_common_http__WEBPACK_IMPORTED_MODULE_1__["HttpClient"] },
...
var ConfigurationComponent = /** @class */ (function () {
    function ConfigurationComponent(commonSvc, utilitySvc) {
        this.commonSvc = commonSvc;
        this.utilitySvc = utilitySvc;
        this.getOptions();
    }
    ...
    ConfigurationComponent.prototype.getOptions = function () {
        var _this = this;
        this.commonSvc.getOptions().subscribe(function (res) {
            _this.options = res;
        });
    };
    ConfigurationComponent.prototype.update = function (id) {
        this.activeId = id;
    };
    ConfigurationComponent.prototype.updateOpton = function (option) {
        var _this = this;
        this.commonSvc.updateOptions(option).subscribe(function (res) {
            if (res) {
                _this.getOptions();
                _this.utilitySvc.showNotification('success', 'Option updated successfully');
            }
        });
    };
...
var UsersComponent = /** @class */ (function () {
    function UsersComponent(modalService, commonSvc, utilitySvc) {
        this.modalService = modalService;
        this.commonSvc = commonSvc;
        this.utilitySvc = utilitySvc;
    ...
    UsersComponent.prototype.getTitle = function (id) {
        return this.commonSvc.getOptionTitle(id);
    };
    ...
    UsersComponent.prototype.deleteUser = function (id) {
        var _this = this;
        this.commonSvc.deleteUser(id).subscribe(function (isDeleted) {
            if (isDeleted) {
                _this.getUsers();
                _this.utilitySvc.showNotification('success', 'Congratulation! Deleted Successfully');
            }
            else {
                _this.utilitySvc.showNotification('danger', "Alert! " + _this.getTitle(2) + " is already in use.");
            }
        });
    };
    UsersComponent.prototype.open = function (content, type, modalDimension, userOption, user) {
        var _this = this;
        this.operationOption = userOption;
        if (option.EDIT == userOption) {
            var headerTitle = "EDIT " + this.getTitle(2);
            var buttonTitle = "Save changes";
            this.modelDialog = { headerTitle: headerTitle, buttonTitle: buttonTitle };
            this.user = user;
        }
        if (option.ADD == userOption) {
            var headerTitle = "ADD " + this.getTitle(2);
            var buttonTitle = "Save";
            this.modelDialog = { headerTitle: headerTitle, buttonTitle: buttonTitle };
            this.user = new _services_Data_Users__WEBPACK_IMPORTED_MODULE_4__["User"]();
        }
        if (option.DETAIL == userOption) {
            var headerTitle = this.getTitle(2) + " DETAIL";
            var buttonTitle = "";
            this.modelDialog = { headerTitle: headerTitle, buttonTitle: buttonTitle };
            this.user = user;
        }
    ...
    UsersComponent.prototype.updateUser = function () {
        var _this = this;
        this.commonSvc.updateUser(this.user).subscribe(function (isUpdated) {
            if (isUpdated) {
                _this.getUsers();
                _this.modalService.dismissAll();
                _this.utilitySvc.showNotification('success', 'Record updated successfully!');
    ...
    UsersComponent.prototype.addUser = function () {
        var _this = this;
        this.commonSvc.addUser(this.user).subscribe(function (isAdded) {
            if (isAdded) {
                _this.getUsers();
                _this.modalService.dismissAll();
                _this.utilitySvc.showNotification('success', 'Record added successfully!');
    ...
    UsersComponent.prototype.showDetail = function () {
        alert('ok');
    };
...
var CommonService = /** @class */ (function () {
    function CommonService(router, utilitySvc) {
        this.router = router;
        this.utilitySvc = utilitySvc;
    }
    CommonService.prototype.initializeData = function () {
        debugger;
    ...
    CommonService.prototype.getOption = function (id) {
        var data = JSON.parse(localStorage.getItem('options'));
        var result;
        for (var i = 0; i < data.length; i++) {
            if (data[i].id == id) {
    ...
    CommonService.prototype.updateOptions = function (option) {
        var data = JSON.parse(localStorage.getItem('options'));
        for (var i = 0; i < data.length; i++) {
    ...
    CommonService.prototype.getOptionTitle = function (id) {
        var data = JSON.parse(localStorage.getItem('options'));
        var result;
        for (var i = 0; i < data.length; i++) {
            if (data[i].id == id) {
    ...
    CommonService.prototype.addUser = function (user) {
        ...
        var maxIDNumber = Math.max.apply(Math, allIds);
    ...
    CommonService.prototype.deleteUser = function (id) {
        var data = JSON.parse(localStorage.getItem('users'));
        var orgdata = JSON.parse(localStorage.getItem('orgChart'));
    ...
    CommonService.prototype.getGroups = function () {
        var data = JSON.parse(localStorage.getItem('Groups'));
        return rxjs__WEBPACK_IMPORTED_MODULE_2__["Observable"].of(data);
    };
    CommonService.prototype.getGroup = function (id) {
        var data = JSON.parse(localStorage.getItem('Groups'));
        var result;
        for (var i = 0; i < data.length; i++) {
            if (data[i].id == id) {
    ...
    CommonService.prototype.deleteGroup = function (id) {
        var data = JSON.parse(localStorage.getItem('Groups'));
        var orgdata = JSON.parse(localStorage.getItem('orgChart'));
    ...
    CommonService.prototype.addSubGroup = function (group, subgrupid) {
        debugger;
        //------------------------------this data will save inside addgrp array---------------------------- //
        var addgrpdata = JSON.parse(localStorage.getItem('addtogrp'));
        ...
        if (addgrpdata.length > 0) {
            for (var i = 0; i < addgrpdata.length; i++) {
        ...
        if (data.length > 0) {
            ...
            var maxIDNumber_1 = Math.max.apply(Math, allIds);
            group.id = maxIDNumber_1 + 1;
        }
        else {
            group.id = 1;
        }
    ...
    CommonService.prototype.addGroup = function (group) {
        debugger;
        //------------------------------this data will save inside addgrp array---------------------------- //
        var addgrpdata = JSON.parse(localStorage.getItem('addtogrp'));
        var users = JSON.parse(localStorage.getItem('users'));
        ...
        if (addgrpdata.length > 0) {
            for (var i = 0; i < addgrpdata.length; i++) {
                if (addgrpdata[i].id < 999) {
                    id[i] = addgrpdata[i].id;
                    maxIDNumber = Math.max.apply(Math, id) + 1;
                }
            }
            debugger;
            addgrpdata.id = maxIDNumber;
        ...
        if (data.length > 0) {
            ...
            var maxIDNumber_2 = Math.max.apply(Math, allIds);
            group.id = maxIDNumber_2 + 1;
        }
        else {
            group.id = 1;
        }
    ...
    CommonService.prototype.updateGroup = function (group) {
        var data = JSON.parse(localStorage.getItem('Groups'));
        for (var i = 0; i < data.length; i++) {
    ...
    CommonService.prototype.getOrgChart = function (id) {
        debugger;
        var data = JSON.parse(localStorage.getItem('orgChart'));
        var result;
        for (var i = 0; i < data.length; i++) {
            if (data[i].id == id) {
                result = data[i];
    ...
    CommonService.prototype.getDemoChart = function (id) {
        debugger;
        var data = JSON.parse(localStorage.getItem('newOrgData'));
        var result;
        for (var i = 0; i < data.length; i++) {
            if (data[i].id == id) {
                result = data[i];
    ...
    CommonService.prototype.addToGroup = function (UserID, groupId) {
        var userid = +UserID;
        userid = userid + 1000;
        try {
            debugger;
            var user = JSON.parse(localStorage.getItem('users'));
            var group = JSON.parse(localStorage.getItem('Groups'));
            var filteredUser;
            var filteredGroup;
    ...
    CommonService.prototype.addToOrganization = function (userId, orgId, groupId) {
        try {
            var user = JSON.parse(localStorage.getItem('users'));
            var group = JSON.parse(localStorage.getItem('Groups'));
            var filteredUser;
    ...
    CommonService.prototype.removeFromOrganization = function (id) {
        var data = JSON.parse(localStorage.getItem('orgChart'));
        var newData = data.filter(function (item) {
            return item.id != id;
        });
    ...
    CommonService.prototype.getUsersForOrganization = function () {
        var users = JSON.parse(localStorage.getItem('users'));
        var data = JSON.parse(localStorage.getItem('orgChart'));
        var allUsersIds = lodash__WEBPACK_IMPORTED_MODULE_3__["map"](users, 'id');
        var allOrgIds = lodash__WEBPACK_IMPORTED_MODULE_3__["map"](data, 'id');
        var ids = allUsersIds.filter(function (e) { return !allOrgIds.includes(e); });
        var result = users.filter(function (e) { return ids.includes(e.id); });
        return rxjs__WEBPACK_IMPORTED_MODULE_2__["Observable"].of(result);
    };
    CommonService.prototype.getOrganizationLevel = function () {
        var data = JSON.parse(localStorage.getItem('orgChart'));
        var allIds = lodash__WEBPACK_IMPORTED_MODULE_3__["map"](data, 'pid');
        var uniqIds = Array.from(new Set(allIds));
...
var GroupsFilter = /** @class */ (function () {
    function GroupsFilter() {
    }
    GroupsFilter.prototype.transform = function (items, filter) {
        if (!items || !filter) {
            return items;
        }
...
var MyFilter = /** @class */ (function () {
    ...
    MyFilter.prototype.transform = function (items, filter) {
        if (!items || !filter) {
            return items;
        }
        return items.filter(function (item) { return item.name.toLowerCase().indexOf(filter.toLowerCase()) != -1 || item.email.toLowerCase().indexOf(filter.toLowerCase()) != -1 || item.phone.toLowerCase().indexOf(filter.toLowerCase()) != -1; });
    };
    MyFilter = __decorate([
        Object(_angular_core__WEBPACK_IMPORTED_MODULE_0__["Pipe"])({
            name: 'myfilter',
...
var UtilityService = /** @class */ (function () {
    ...
    UtilityService.prototype.showNotification = function (type, msg) {
        var from = 'top';
        var align = 'right';
        $.notify({
            icon: "notifications",
